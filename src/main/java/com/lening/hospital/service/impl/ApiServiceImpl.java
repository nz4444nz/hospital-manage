package com.lening.hospital.service.impl;

import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.lening.hospital.mapper.HospitalSetMapper;
import com.lening.hospital.mapper.ScheduleMapper;
import com.lening.hospital.model.HospitalSet;
import com.lening.hospital.model.Schedule;
import com.lening.hospital.service.ApiService;
import com.lening.hospital.util.BeanUtils;
import com.lening.hospital.util.HttpRequestHelper;
import com.lening.hospital.util.MD5;
...
public class ApiServiceImpl implements ApiService {
    ...
    public JSONObject getHospital() {
        ...
        //这个是在医院这一边查询出来的签名key，进行加密处理，到了平台，平台会根据
        //医院编码，去查询出来签名的key，然后和这边使用一样的加密方式，进行加密，
        //加密完成后，和医院传递的加密后的key进行比较，一样就是鉴权通过
        //两端的加密方式一样，就是使用同一种方式，并且，加密文件都要一样，或者说算法一样
        paramMap.put("sign", MD5.encrypt(this.getSignKey()));
        /**
         * /api/hosp/hospital/show 接下来我们进行开发的接口
         * 应该写在配置文件里面，动态去读取。admin/hosp/api/hospital
         */
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/admin/hosp/api/hospital/show");
    ...
    public boolean saveHospital(String data) {
         ...
         */
        JSONObject respone =
                HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/admin/hosp/api/hospital/saveHospital");
        System.out.println(respone.toJSONString());
    ...
    public Map<String, Object> findDepartment(int pageNum, int pageSize) {
        ...
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        /**
         * 这个签名使用了不同的加密方法。不是md5的。有可能采坑
         */
        paramMap.put("sign", MD5.encrypt(this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/admin/hosp/api/department/list");
        if(null != respone && 200 == respone.getIntValue("code")) {
            JSONObject jsonObject = respone.getJSONObject("data");
    ...
    public boolean saveDepartment(String data) {
        ...
        for(int i=0, len=jsonArray.size(); i<len; i++) {
            ...
            paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
            paramMap.put("sign",MD5.encrypt(this.getSignKey()));
            JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/admin/hosp/api/department/saveDepartment");
            System.out.println(respone.toJSONString());
    ...
    public boolean removeDepartment(String depcode) {
        ...
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", MD5.encrypt(this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/admin/hosp/api/department/remove");
        System.out.println(respone.toJSONString());
        if(null != respone && 200 == respone.getIntValue("code")) {
            return true;
        } else {
    ...
    public Map<String, Object> findSchedule(int pageNum, int pageSize) {
        ...
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", MD5.encrypt(this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/admin/hosp/api/schedule/list");
        System.out.println(respone.toJSONString());
        if(null != respone && 200 == respone.getIntValue("code")) {
            JSONObject jsonObject = respone.getJSONObject("data");
            result.put("total", jsonObject.getLong("totalElements"));
    ...
    public boolean saveSchedule(String data) {
        ...
        for(int i=0, len=jsonArray.size(); i<len; i++) {
            ...
            JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/admin/hosp/api/schedule/saveSchedule");
            System.out.println(respone.toJSONString());
            if(null == respone || 200 != respone.getIntValue("code")) {
                throw new YyghException(respone.getString("message"), 201);
            }
    ...
    public boolean removeSchedule(String hosScheduleId) {
        ...
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", MD5.encrypt(this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/admin/hosp/api/schedule/remove");
        System.out.println(respone.toJSONString());
        if(null != respone && 200 == respone.getIntValue("code")) {
            return true;
        } else {
    ...
    public void  saveBatchHospital() throws IOException {
        ...
        for(int i=1, len=jsonArray.size(); i<len; i++) {
            ...
            JSONObject respone = HttpRequestHelper.sendRequest(paramMap,"http://localhost/admin/hosp/api/schedule/saveHospital");
            System.out.println(respone.toJSONString());
            if(null == respone || 200 != respone.getIntValue("code")) {
                throw new YyghException(respone.getString("message"), 201);
            }
